...
class IteratorRange {
 public:
  IteratorRange(const It &f, const It &l, size_t s) : first{f}, last{l}, page_size{s} {}
 ...
 public:
  Paginator(const It &begin, const It &end, size_t page_size) {
	for (auto it = begin; it < end; it += page_size) {
	  auto cend = it + page_size > end ? end : it + page_size;
	  pages.emplace_back(it, cend, static_cast<size_t>(distance(it, cend)));
	}
  }
