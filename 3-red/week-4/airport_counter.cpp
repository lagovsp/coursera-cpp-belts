...
class AirportCounter {
 public:
  AirportCounter() { array_.fill(0); }

  template<typename TIterator>
  AirportCounter(TIterator begin, TIterator end) {
	array_.fill(0);
	for (auto it = begin; it != end; ++it) {
	  ++array_[static_cast<size_t>(*it)];
	}
  }

  [[nodiscard]] size_t Get(TAirport airport) const {
	return array_[static_cast<size_t>(airport)];
  }

  void Insert(TAirport airport) {
	++array_[static_cast<size_t>(airport)];
  }

  void EraseOne(TAirport airport) {
	auto n = static_cast<size_t>(airport);
	auto outs = array_[n];
	array_[n] = (outs == 0) ? 0 : --outs;
  }

  void EraseAll(TAirport airport) {
	array_[static_cast<size_t>(airport)] = 0;
  }
  ...
  [[nodiscard]] Items GetItems() const {
	...
	for (size_t i = 0; i < array_.size(); ++i) {
	  is[i] = {static_cast<TAirport>(i), array_[i]};
	}
	return is;
  }

 private:
  array<size_t, static_cast<uint32_t>(TAirport::Last_)> array_;
};
...
void TestManyConstructions() {
  ...
  for (int step = 0; step < 100'000'000; ++step) {
	AirportCounter<SmallCountryAirports>
		counter(begin(airports), end(airports));
...
void TestMostPopularAirport() {
  ...
  ASSERT(all_of(begin(most_popular),
				end(most_popular),
				[&](SmallCountryAirports a) {
