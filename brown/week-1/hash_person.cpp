...
struct Person {
  ...
  bool operator==(const Person &rhs) const {
	return name == rhs.name
		&& height == rhs.height
		&& abs(weight - rhs.weight) < 0.001
		&& address == rhs.address;
  }
};

struct AddressHasher {
  size_t operator()(const Address &rhs) const {
	const hash<string> strHasher;
	const hash<int> intHasher;
	const size_t coeff = 2'946'901;

	return strHasher(rhs.city) * coeff * coeff
		+ strHasher(rhs.street) * coeff
		+ intHasher(rhs.building);
  }
};
...
struct PersonHasher {
  size_t operator()(const Person &rhs) const {
	const hash<string> strHasher;
	const hash<int> intHasher;
	const hash<double> doubHasher;
	const AddressHasher addressHasher;
	const size_t coeff = 2'946'901;

	return strHasher(rhs.name) * coeff * coeff * coeff
		+ intHasher(rhs.height) * coeff * coeff
		+ doubHasher(rhs.weight) * coeff
		+ addressHasher(rhs.address);
  }
};
